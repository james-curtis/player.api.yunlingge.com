...
var lzxPlayerInit = function () {
    ...
    var jsUrl = jQuery("#ilt").attr("src");
    var webURL = jsUrl.startsWith("http") ? jsUrl.substring(0,jsUrl.indexOf("/",8)) : window.location.origin;
    var keyId = isTest ? testKey : jQuery("#ilt").attr("key");

    if(!styleLoaded){
        jQuery("head").append('<link rel="stylesheet" type="text/css" href="' + webURL + '/player/css/player.css">');
        jQuery("head").append('<link href="https://libs.baidu.com/fontawesome/4.2.0/css/font-awesome.css" rel="stylesheet" type="text/css">');
        lzxPlayerStyleLoaded = true;
    }

    jQuery("body").append('<div id="lzxPlayer">\n' +
        '    <div class="player">\n' +
    ...
    var audio = new Audio(),
        $player = jQuery('#lzxPlayer'),
        $tips = jQuery('#lzxTips'),
        $lk = jQuery('#lzxKsc,#lzxLrc'),
        $switchPlayer = jQuery('.switch-player', $player),
        $btns = jQuery('.status', $player),
        $songName = jQuery('.song', $player),
        $cover = jQuery('.cover', $player),
        $songTime = jQuery('.time', $player),
        $songList = jQuery('.song-list .list', $player),
        $albumList = jQuery('.album-list', $player),
        $songFrom = jQuery('.player .artist', $player),
        $songFrom1 = jQuery('.player .artist1', $player),
        $songFrom2 = jQuery('.player .moshi', $player),
        $songFrom3 = jQuery('.player .geci', $player),
        $songFrom4 = jQuery('.player .switch-ksclrc', $player),
        $volumeSlider = jQuery('.volume-controls .slider', $player);
        $rateBuffered = jQuery('.musicbottom .rate-buffered', $player);
        $rateSlider = jQuery('.rate .progress', $player);
        songFrom33 = '开启',
        songFrom44 = '',
        songFrom55 = '',
        roundcolor = '#6c6971',
        ...
        ycgeci = true,
        first = 1,
        volume = jQuery.cookie('lzx_player_volume') ? jQuery.cookie('lzx_player_volume') : '0.666',
        albumId = 0,
        songId = 0,
        songTotal = 0,
        random = true,
    ...
    if(isPhone){jQuery('#lzxLrc').addClass('phone');$player.addClass('phone');jQuery(".new-volume",$player).hide();}
    ...
    window.lzxPlayerReload = function () {
        ...
        jQuery("#lzxPlayer").remove();
        jQuery("#lzxLrc").remove();
        jQuery("#lzxTips").remove();
        audio.pause();
    };
    ...
    var lzxMedia = {
        play: function () {
            ...
            cicleTime = setInterval(function(){
                if(!rateMouse.isDown && !rateTouch.isTouchDown){
                    ...
                    jQuery(".rate-on",$rateSlider).width(audio.currentTime / audio.duration * 100+"%");
                }
            ...
            if (hasLrc) {
                ...
                jQuery('#lzxLrc').addClass('show');
                jQuery('.switch-down').css('right', '65px');
                jQuery('.switch-ksclrc').show()
            }
        },
        pause: function () {
            clearInterval(cicleTime);
            $player.removeClass('playing');
            jQuery('.switch-ksclrc').hide();
            jQuery('.switch-down').css('right', '35px');
            if (hasLrc) {
                lzxLrc.lrc.hide()
            }
        },
        ...
        volumechange: function () {
            ...
            jQuery('.progress2', $volumeSlider).height(vol + '%');
            lzxTips.show('音量：' + vol + '%');
            jQuery.cookie("lzx_player_volume", audio.volume);
        },
    ...
    var lzxTips = {
        show: function (cont) {
            ...
            jQuery('#lzxTips').text(cont).addClass('show');
            this.hide()
        },
        hide: function () {
            lzxTipsTime = setTimeout(function () {
                jQuery('#lzxTips').removeClass('show');
            }, 3000)
        }
    };
    ...
    //音乐暂停事件
    jQuery('.pause', $player).click(function () {
        hasgeci = false;
        jQuery("li", $albumList).eq(albumId).addClass(cur).find(".artist").html("暂停播放 > ").parent().siblings()
        ...
        jQuery.cookie("auto_playre", "no");
    });
    //音乐播放事件
    jQuery('.play', $player).click(function () {
        hasgeci = true;
        jQuery('#lzxLrc,#lzxKsc').show();
        jQuery("li", $albumList).eq(albumId).addClass(cur).find(".artist").html("当前播放 > ").parent().siblings()
            .removeClass(cur).find(".artist").html("").parent();
        startPlay();
        setTimeout(function () {
            lzxTips.show("播放器下次访问将自动播放");
        }, 4000);
        jQuery.cookie("auto_playre", "yes");
    });
    //上一首事件
    jQuery('.prev', $player).click(function () {
        hasgeci = true;
        jQuery('#lzxLrc,#lzxKsc').show();
        lzxMedia.prev();
        jQuery.cookie("auto_playre", "yes");
    ...
    //下一首事件
    jQuery('.next', $player).click(function () {
        hasgeci = true;
        jQuery('#lzxLrc,#lzxKsc').show();
        lzxMedia.next();
        jQuery.cookie("auto_playre", "yes");
    ...
    //随机播放按钮事件
    jQuery('.random', $player).click(function () {
        jQuery(this).addClass(cur);
        jQuery('.loop', $player).removeClass(cur);
        random = true;
        lzxTips.show('随机播放');
        $songFrom2.html('<i class="random fa fa-random current"></i> 随机播放')
        jQuery.cookie("random_play", true)
    });
    //顺序播放按钮事件
    jQuery('.loop', $player).click(function () {
        jQuery(this).addClass(cur);
        jQuery('.random', $player).removeClass(cur);
        random = false;
        lzxTips.show('顺序播放');
        $songFrom2.html('<i class="loop fa fa-retweet"></i> 顺序播放')
        jQuery.cookie("random_play", false)
    });
    //音量组件拖动事件
    ...
    var isDown = false;
    jQuery('.drag', $volumeSlider).mousedown(function () {
        isDown = true;
    });

    jQuery('.drag', $rateSlider).on("touchstart",(function (e) {
        rateTouch.progressWidth = $rateSlider.width();
        rateTouch.isTouchDown = true;
        rateTouch.startX = e.originalEvent.touches[0].clientX;
        rateTouch.rateOnWidth = parseFloat((jQuery(".rate-on",$rateSlider).width() / rateTouch.progressWidth).toFixed(2));
    }));

    jQuery('.drag', $rateSlider).mousedown(function (e) {
        rateMouse.progressWidth = $rateSlider.width();
        rateMouse.isDown = true;
        rateMouse.startX = e.clientX;
        rateMouse.rateOnWidth = parseFloat((jQuery(".rate-on",$rateSlider).width() / rateMouse.progressWidth).toFixed(2));
    });
    ...
    window.addEventListener("touchmove",function(e){
        if(rateTouch.isTouchDown){
            ...
            if(rate >= 0 && rate <= 1){
                jQuery(".rate-on",$rateSlider).width(rate * 100 + '%');
    ...
    //播放列表按钮点击事件
    jQuery('.switch-playlist').click(function () {
        $player.toggleClass('showAlbumList')
    });
    //返回专辑列表事件
    $songList.mCustomScrollbar();
    jQuery('.song-list .musicheader,.song-list .fa-angle-right', $player).click(function () {
        $player.removeClass('showSongList')
    });
    //打开关闭歌词显示
    jQuery('.switch-ksclrc').click(function () {
        $player.toggleClass('ksclrc');
        jQuery('#lzxLrc').toggleClass('hide');
        jQuery('#lzxKsc').toggleClass('hidePlayer');
        if (!jQuery('#lzxLrc').hasClass('hide')) {
            ycgeci = true;
            if (hasLrc) {
    ...
    lzxPlayer.playList = {
        creat: {
            album: function () {
                jQuery('.musicheader', $albumList).html(siteName + ' - 专辑列表');
                ...
                jQuery('.list', $albumList).html('<ul>' + albumList + '</ul>').mCustomScrollbar();

                jQuery("li", $albumList).click(function () {
                    var a = jQuery(this).index();
                    jQuery(this).hasClass(cur) ? lzxPlayer.playList.creat.song(a, true)
            ...
            song: function (id, isThisAlbum) {
                ...
                jQuery(".song-list .musicheader span", $player)
                ...
                jQuery('ul', $songList).html(songList);
                $songList.mCustomScrollbar('update');
                if (isThisAlbum) {
                    jQuery("li", $songList).eq(songId).addClass(cur).siblings().removeClass(cur);
                    $songList.mCustomScrollbar("scrollTo", jQuery("li.current", $songList).position().top - 120);
                } else {
                    $songList.mCustomScrollbar("scrollTo", "top");
                }
                jQuery('li', $songList).click(function () {
                    hasgeci = true;
                    jQuery('#lzxLrc,#lzxKsc').show();
                    albumId = id;
    ...
    var lzxLrc = {
        ...
        lrc: {
            format: function (cont) {
                ...
                jQuery('#lzxLrc').html('<ul>' + lrcLine + '</ul>');
                setTimeout(function () {
                        if (audio.paused) {
                            jQuery('.switch-ksclrc').hide();
                            jQuery('.switch-down').css('right', '35px');
            ...
            hide: function () {
                ...
                jQuery('#lzxLrc').removeClass('show')
            }
        }
    };
    //设置默认音量
    audio.volume = volume;
    if (volume == 1) {
        jQuery('.volume-on', $player).width('100%');
    }
    //获取歌单列表数据
    jQuery.ajax({
        ...
        success: function () {
            ...
            if(showNotes !== 1){
                jQuery(".status .note",$player).hide()
            }
            ...
            if (jQuery.cookie("random_play") != null) {
                if (jQuery.cookie("random_play") == "true") {
                    jQuery('.loop', $player).removeClass(cur);
                    jQuery('.random', $player).addClass(cur);
                    $songFrom2.html('<i class="random fa fa-random"></i> 随机播放');
                    random = true;
                } else {
                    jQuery('.loop', $player).addClass(cur);
                    jQuery('.random', $player).removeClass(cur);
                    $songFrom2.html('<i class="loop fa fa-retweet"></i> 顺序播放');
                    random = false;
                }
            ...
            if (jQuery.cookie("lzx_player_volume") == '0.666') {
    ...
    function netmusic() {
        ...
        jQuery('.switch-down').show();
        jQuery('.switch-down').html('<a class="down"><i class="fa fa-cloud-download" title="从' + songFrom55 + '下载：'
        ...
        $cover.html(coverImg);
        if (background == 1) {
            jQuery('.blur-img .blur', $player).attr("src", songSheetList[albumId].albumCovers[songId]); //虚化背景
        } else {
            if (jQuery(".blur-img").length > 0) {
                jQuery(".blur-img").remove();
            }
        }
        lzxLrc.load(); //加载歌词
        ...
        if (first == 1) {
            first = 2;
            if (autoPlayer == 1 && (jQuery.cookie("auto_playre") == null || jQuery.cookie("auto_playre") === "yes")) {
                startPlay()
            } else {
        ...
        jQuery(window).scroll(function () {
            ...
            var scrollHeight = jQuery(window.document).height();
            var windowHeight = jQuery(this).height();
            if (scrollTop + windowHeight == scrollHeight) {
                if (hasgeci && ycgeci) {
                    $player.addClass('ksclrc');
                    jQuery('#lzxLrc').addClass('hide');
                    jQuery('#lzxKsc').addClass('hidePlayer');
                ...
                if (hasgeci && ycgeci) {
                    ...
                    jQuery('#lzxLrc').removeClass('hide');
                    jQuery('#lzxKsc').removeClass('hidePlayer');
                    if (hasLrc) {
                        $songFrom3.html('<i class="fa fa-check-circle"></i> 歌词开启')
                    }
                    $songFrom4.html('<i class="fa fa-toggle-on" title="关闭歌词"></i>')
    ...
    function allmusic() {
        ...
        if (jQuery('ul', $songList).html() != '') $songList.mCustomScrollbar("scrollTo", jQuery("li.current", $songList)
            .position().top - 120);
    }
    ...
    function playerColor() {
        ...
        jQuery(".infos,.control,.status .note", $player).css({
    ...
    function musicTooltip() {
        if (isPhone){return;}
        jQuery('#lzxPlayer span,#lzxPlayer i').each(function () {
            jQuery('#tooltip').remove();
            if (this.title) {
                var a = this.title;
                jQuery(this).unbind("mouseover mouseout");
                jQuery(this).mouseover(function (b) {
                    this.title = '';
                    jQuery('body').append('<div id="tooltip">' + a + '</div>');
                    jQuery('#tooltip').css({
                        left: b.pageX - 15 + 'px',
                        top: b.pageY + 30 + 'px',
                        opacity: '0.8'
